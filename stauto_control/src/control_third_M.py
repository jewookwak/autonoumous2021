...
def delivery_path_callback(data):
    global delivery_path

    for i in range(len(data.poses)):
        delivery_path[i][0] = data.poses[i].pose.position.x
        delivery_path[i][1] = data.poses[i].pose.position.y

def big_obstacle_path_callback(data):
    global big_obstacle_path

    for i in range(len(data.poses)):
        big_obstacle_path[i][0] = data.poses[i].pose.position.x
        big_obstacle_path[i][1] = data.poses[i].pose.position.y
...
def delivery_finish_callback(data):
    global delivery_finish_sign

    delivery_finish_sign=data.data

def big_obstacle_finish_callback(data):
    global big_obstacle_finish_sign

    big_obstacle_finish_sign=data.data
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/delivery_path",Path,delivery_path_callback)
    rospy.Subscriber("/big_obstacle_path",Path,big_obstacle_path_callback)
    ...
    rospy.Subscriber("/delivery_finish", Bool, delivery_finish_callback)
    rospy.Subscriber("/big_obstacle_finish", Bool, big_obstacle_finish_callback)
    ...
    delivery_end_pub = rospy.Publisher('/delivery_end', Bool, queue_size=10)
    big_obstacle_end_pub = rospy.Publisher('/big_obstacle_end', Bool, queue_size=10)
    ...
    delivery_path = np.zeros((100,2))
    big_obstacle_path = np.zeros((100,2))
    local_path_length=0
    ...
    state_machine = np.zeros(7)
    prev_state_machine = np.zeros(7)
    ...
    delivery_finish_sign = False
    big_obstacle_finish_sign = False
    going_gps_n=[0,0]
    going_gps_n1=[0,0]
    going_gps_n2=[0,0]
    going_gps_n3=[0,0]
    ...
    while not rospy.is_shutdown():
        ...
        if(state_machine[1]==1):
            going_gps_n[0]=big_obstacle_path[0][0]
            going_gps_n[1]=big_obstacle_path[0][1]
            going_gps_n1[0]=big_obstacle_path[1][0]
            going_gps_n1[1]=big_obstacle_path[1][1]
            going_gps_n2[0]=big_obstacle_path[2][0]
            going_gps_n2[1]=big_obstacle_path[2][1]
            going_gps_n3[0]=big_obstacle_path[3][0]
            going_gps_n3[1]=big_obstacle_path[3][1]
        ...
        elif(state_machine[5]==1):
            going_gps_n[0]=delivery_path[0][0]
            going_gps_n[1]=delivery_path[0][1]
            going_gps_n1[0]=delivery_path[1][0]
            going_gps_n1[1]=delivery_path[1][1]
            going_gps_n2[0]=delivery_path[2][0]
            going_gps_n2[1]=delivery_path[2][1]
            going_gps_n3[0]=delivery_path[3][0]
            going_gps_n3[1]=delivery_path[3][1]
        ...
        if(state_machine[0]==1):
            ...
            if big_obstacle_finish_sign==False:
                ...
                big_obstacle_end_pub.publish(True)
            ...
            if delivery_finish_sign==False:
                ...
                if (time.time()-delivery_finish_time<4):
                    ackermann.drive.speed = 0.0
                    ackermann.drive.steering_angle = 0.0
                    ackermann.drive.jerk = 100
                    ackermann.drive.acceleration = 0
                    print(1)
                elif (time.time()-delivery_finish_time<8.5):
                    ackermann.drive.speed = 3.0
                    ackermann.drive.steering_angle = 0.0
                    ackermann.drive.jerk = 0
                    ackermann.drive.acceleration = 2
                    print(2)
                elif (time.time()-delivery_finish_time<10.2):
                    ackermann.drive.speed = 3.0
                    ackermann.drive.steering_angle = -28.0*np.pi/180
                    ackermann.drive.jerk = 0
                    ackermann.drive.acceleration = 2
                    print(3)
                elif (time.time()-delivery_finish_time<10.5):
                    ackermann.drive.speed = 0.0
                    ackermann.drive.steering_angle = 0.0
                    ackermann.drive.jerk = 100
                    ackermann.drive.acceleration = 0
